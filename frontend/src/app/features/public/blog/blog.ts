...
@Component({
  selector: 'app-blog',
  standalone: true,
  imports: [CommonModule, FormsModule, TranslocoModule, TimeAgoPipe, AskQuestionModal],
...
this.loadMajors();
    ...
    this.blogApi.topContributors().subscribe({ next: (res) => (this.topContributors = res.data) });
  }

  private loadMajors(): void {
    this.catalogApi.getMajors({ page: 1, per_page: 100 }).subscribe({
      next: (res) => (this.majors = res.data.items),
    });
  }
  ...
  if (this.selectedMajorId) {
    this.catalogApi.getSubjects({ major_id: this.selectedMajorId, page: 1, per_page: 200 }).subscribe({
      next: (res) => (this.subjects = res.data.items),
    });
  }
  this.applyFilters();
}
  ...
  get paginationItems(): (number | 'ellipsis')[] {
    ...
    if (start > 1) { items.push(1); if (start > 2) items.push('ellipsis'); }
    for (let i = start; i <= end; i++) items.push(i);
    if (end < total) { if (end < total - 1) items.push('ellipsis'); items.push(total); }
